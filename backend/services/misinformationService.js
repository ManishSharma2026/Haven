...
function buildExplanation(resultType, eventModel, relevantArticles, responseStyle = "medium") {
  ...
  if (resultType === "strong") {
    if (responseStyle === "short") return `Haven found strong corroboration from multiple sources for ${primaryAnchor}.`;
    if (responseStyle === "detailed") return `Haven found strong corroboration across ${uniqueSources} different sources. Multiple articles align with the core actor, action, and object of the claim, and the retrieved coverage appears to describe the same event rather than only a loosely related topic.`;
